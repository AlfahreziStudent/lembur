...
<div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg mb-5">
    <div class="w-full">
        ...
        <form method="post" action="{{ route('tambah-judul') }}" class="mt-6 space-y-6">
            @csrf
            <div class="w-1/2">
                <x-input-label for="judul" :value="__('Judul Laporan')" />
                <x-text-input id="judul" name="judul" type="text" class="mt-1 block w-full" required autofocus autocomplete="judul" />
